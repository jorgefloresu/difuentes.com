...
function print_invoices()
{
	...
	include_once($path_to_root . "/reporting/includes/pdf_report_modif.inc");
	...
	//$cols = array(4, 60, 225, 300, 325, 385, 450, 515);
        $cols = array(10, 30, 40, 100, 125);

	// $headers in doctext.inc
	$aligns = array('right', 'left', 'left', 'right', 'left');
	...
	//	$rep = new FrontReport(_('INVOICE'), "InvoiceBulk", user_pagesize());
		$rep = new FrontReport(_('INVOICE'), "InvoiceBulk", "LETTER");
		$rep->currency = $cur;
		$rep->Font();
		...
		for ($j = ST_SALESINVOICE; $j <= ST_CUSTCREDIT; $j++)
		{
			...
			while ($myrow2=db_fetch($result))
			{
				...
				//$rep->TextCol(0, 1,	$myrow2['stock_id'], -2);
                                $rep->Text(15, $myrow2['StockDescription']);
                                $rep->NewLine();
				...
				$rep->TextCol(2, 3,	"X".$DisplayPrice, -2);
			...
			if ($comments && db_num_rows($comments))
			{
    			...
    			while ($comment=db_fetch($comments))
    				$rep->TextColLines(0, 5, $comment['memo_'], -2);
			}

   			$DisplaySubTot = number_format2($SubTotal,$dec);
   			$DisplayFreight = number_format2($sign*$myrow["ov_freight"],$dec);

    		//$rep->row = $rep->bottomMargin + (19 * $rep->lineHeight) - 10;
			$linetype = true;
			$doctype = $j;
			...
			{
				include($path_to_root . "/reporting/includes/doctext_modif.inc");
			}

			//$rep->TextCol(3, 6, $doc_Sub_total, -2);
                        //$rep->TextCol(6, 7,	$DisplaySubTot, -2);
                        //$rep->TextCol(3, 4, $doc_Sub_total, -2);
			//$rep->TextCol(4, 5,	$DisplaySubTot, -2);
			//$rep->NewLine();
			//$rep->TextCol(3, 4, $doc_Shipping, -2);
    		...
    		while ($tax_item = db_fetch($tax_items))
    		{
    			...
    			if ($tax_item['included_in_price'])
    			{
					...
					//$rep->TextCol(3, 4, $doc_Included . " " . $tax_item['tax_type_name'] .
					//	" (" . $tax_item['rate'] . "%) " . $doc_Amount . ": " . $DisplayTax, -2);
				}
    			else
    			{
					//$rep->TextCol(3, 4, $tax_item['tax_type_name'] . " (" .
					//	$tax_item['rate'] . "%)", -2);
			...
			$rep->Font('bold');
                        $rep->row = $rep->row - 5;
			//$rep->TextCol(2, 3, $doc_TOTAL_INVOICE, - 2);
                        $rep->TextCol(2, 3, "TOTAL VENTA", - 2);
