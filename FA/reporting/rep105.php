 ...
 */
        $sql= "SELECT ".TB_PREF."sales_orders.order_no,
                ...
                ".TB_PREF."stock_master.description,
                ".TB_PREF."stock_master.units,
                sum(".TB_PREF."sales_order_details.quantity) quantity,
                ".TB_PREF."sales_order_details.unit_price,
                sum((".TB_PREF."sales_order_details.quantity*".TB_PREF."sales_order_details.unit_price)
                    -(".TB_PREF."sales_order_details.quantity*".TB_PREF."sales_order_details.unit_price
                        *".TB_PREF."sales_order_details.discount_percent)) total_product,    
                sum(".TB_PREF."sales_order_details.qty_sent) qty_sent,
	...
	if ($location > 0)
		$sql .= " AND ".TB_PREF."cust_branch.salesman=".db_escape($location);
	if ($backorder)
		$sql .= " AND ".TB_PREF."sales_order_details.quantity - ".TB_PREF."sales_order_details.qty_sent > 0";
        
        $sql .= " GROUP BY ".TB_PREF."cust_branch.salesman, ".TB_PREF."sales_order_details.stk_code";
	$sql .= " ORDER BY ".TB_PREF."cust_branch.salesman, ".TB_PREF."sales_order_details.stk_code";
...
function print_order_status_list()
{
	...
	$cols = array(0, 60, 210, 260, 325,	385, 450, 515);
	...
	//	_('Outstanding'), '');
        $headers = array(_('Code'),	_('Description'), _('Cantidad'),	_('Precio'),
		_('Total'), '');
	...
	while ($myrow=db_fetch($result))
	{
                ...
                if ($orderno != $myrow['salesman'])
		{
			if ($orderno != 0)
			{
				...
				$rep->NewLine();
                                ...
                                $rep->AmountCol(4, 5,   $total_vendedor, 3);
                                $rep->NewLine();
                                $rep->Line($rep->row);
                                $rep->NewLine();
                                $total_vendedor = 0;
			...
			//$rep->TextCol(2, 3,	get_branch_name($myrow['branch_code']));
                        $rep->TextCol(0, 1,	"Vendedor:");
                        $rep->TextCol(1, 2,	$myrow['salesman_name']);
                        $rep->TextCol(2, 3,	$myrow['customer_ref']);
		...
		//$rep->AmountCol(3, 4, $myrow['qty_sent'], $dec);
                $rep->AmountCol(3, 4, $myrow['unit_price'], 3);
		//$rep->AmountCol(4, 5, $myrow['quantity'] - $myrow['qty_sent'], $dec);
                $rep->AmountCol(4, 5, $myrow['total_product'], 3);
		...
		if ($rep->row < $rep->bottomMargin + (2 * $rep->lineHeight))
		{
			...
			$rep->Header();
                ...
                $total_vendedor += $myrow['total_product'];
	}
